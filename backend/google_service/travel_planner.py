...
def route_flight_api(entities: Dict, query: str = ""):
    """Returns (decision, src, dest)."""
    to_city   = entities.get("to_city")
    from_city = entities.get("from_city")

    # Skip flights for road/drive trips
    road_trip_keywords = ("road trip", "drive", "driving", "car trip", "by road", "by car", "road")
    if any(kw in query.lower() for kw in road_trip_keywords):
...
out center tags 100;"""
        r = requests.post(OVERPASS_URL, data={"data": q},
                          headers={"User-Agent": "OrbixAI/1.0"}, timeout=15)
        r.raise_for_status()
...
def generate_itinerary(city: str, attractions: List[Dict], num_days: int,
    ...
    Model priority:
      1. phi4-mini  (travel-optimised SLM, runs locally via Ollama)
      2. generate_response()  (existing Llama fallback — unchanged)
    """
    # ── 1. Try phi4-mini first ────────────────────────────────────────────
    try:
        from llm.phi4_mini_client import generate_itinerary_phi4
        result = generate_itinerary_phi4(
            city=city,
        ...
        if result:
            logger.info("[Travel] Itinerary generated by phi4-mini")
            return result
        logger.warning("[Travel] phi4-mini returned nothing — falling back to Llama")
    except Exception as e:
        logger.warning("[Travel] phi4-mini unavailable (%s) — falling back to Llama", e)

    # ── 2. Fallback: existing Llama-based generator ───────────────────────
    ...
    try:
        logger.info("[Travel] Itinerary generated by Llama fallback")
...
def plan_trip(query: str, emit: Callable[[str], None] = None) -> Dict:
    ...
    if not to_city:
        # Let phi4-mini answer using its own knowledge
        from llm.phi4_mini_client import generate_itinerary_phi4
        answer = generate_itinerary_phi4(
            city="the destination mentioned in the query",
            attractions=[],
            num_days=num_nights,
            check_in=check_in or datetime.now().strftime("%Y-%m-%d"),
            num_adults=num_adults,
        )
        return {"itinerary": answer or "Please mention a destination city so I can plan your trip!"}
    result = {"entities": entities, "flights": [], "hotels": [], "attractions": [], "itinerary": ""}

    # Step 2: Flights
    flight_decision, src, dest = route_flight_api(entities, query=query)
    flight_summary = None
